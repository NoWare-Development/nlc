...
public:
  ...
  template <typename T>
  T
  get_argument_value (const std::string &name) const
  {
  ...
  template <typename T>
  std::vector<T>
  get_argument_value_list (const std::string &name) const
  {
    if (!has_argument (name))
      return {};
    ...
    while (commapos != std::string::npos)
      {
        ...
        std::string newvalues
            = values.substr (commapos + 1, std::string::npos);
        values = newvalues;
...
private:
  ...
  bool
  is_dangling (const std::string &arg) const
  {
    return arg.length () > 0 && arg.at (0) != '-';
  }

  Argument
  get_argument (const std::string &arg) const
  {
    if (arg.length () == 0)
      return {};
  ...
  template <typename T,
            typename U = typename std::enable_if<
                ...
                T>::type>
  constexpr T
  string_to_value (const std::string &str) const
  {
